...
const StaffTasks = () => {
  ...
  const downloadExcel = async() =>{
     try {
      const downloadUrl = `${import.meta.env.VITE_API_BASE_URL}${endpoints.exportAllTasksToExcel}?userId=${user.id}`;
      const response = await apiClient.get(downloadUrl, {
        responseType: "blob", // Ensure the response is treated as a binary file
      });
  ...
  const columns = [
    {
      ...
      render: (value, row) => (
        <Link
          to={`${paths.StaffTasks}/view/${row._id}`}
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [filters, currentPage, pageSize, user.id]); // Dependencies for fetchData
  ...
  return (
    <div>
      ...
      <div className="my-4 w-full flex flex-col gap-y-4 lg:flex-row items-center justify-between  ">
        ...
        <h1 className="text-primary tracking-[1px] whitespace-nowrap">Export</h1>
        <select
            value={exportData}
            onChange={(e) =>
          ...
          <button onClick={downloadPDF} className="flex items-center gap-x-1 mr-2 text-red-500"  title={`Export ${exportData} to PDF`} ><BsDownload/>PDF</button>


          <button onClick={downloadExcel} className="flex items-center gap-x-1 text-green-500" title={`Export ${exportData} to Excel`}><BsDownload/>Excel</button>
        </div>

        <div className="flex items-center gap-3 text-sm overflow-x-auto px-3 w-full lg:w-auto ">
          <h4 className="text-sm whitespace-nowrap">Filter By Date</h4>
